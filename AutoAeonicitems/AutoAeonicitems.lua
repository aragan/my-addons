...
local function start_autotrade(group_name)
    ...
    if not group_name then
        windower.add_to_chat(207, "[AutoTrade] Usage: //aai autotrade|at <zitah|ruaun|reisenjima>")
        return
    end
...
windower.register_event('addon command', function(command, ...)
    ...
    if command == 'at' then command = 'autotrade' end
    ...
    if command == 'autotrade' then
        ...
        if not group_name then
            windower.add_to_chat(207, "[AutoTrade] Usage: //aai autotrade|at <zitah|ruaun|reisenjima|zi|ru|re>")
            return
        end
    ...
    elseif command == 'help' then
        ...
        windower.add_to_chat(207, "    //aai autotrade|at <GroupAlias>                - Trade pops to NPC for that group.")
        ...
        windower.add_to_chat(207, "    //aai autotrade|at re")
    ...
    -----------------------------------------------------------
        ...
        windower.add_to_chat(207, "  //aai autotrade|at <GroupAlias>")
...
windower.register_event('prerender', function()
    if running and current_group and os.clock() >= trade_timer then
        ...
        if current_index <= #group_data.nms then
            ...
            ---------------------------------------------------
            --   طريقة التبادل (من AutoTrade):
            --   1) إغلاق أي منيو وحوش مفتوحة (Escape)
            --   2) استهداف الـ NPC
            --   3) تنفيذ أمر TradeNPC
            --   4) الضغط على Down ثم Enter لاختيار أول وحش
            ---------------------------------------------------
            local cmd_str =
                'setkey escape down; wait 0.1; setkey escape up; @wait 0.5;' ..
                'input /targetnpc;@wait 1;' ..
                nm_data.command ..
                ';@wait 3;setkey down down;wait 0.1;setkey down up; ' ..
                'wait 0.1;setkey enter down; wait 0.1; setkey enter up'
